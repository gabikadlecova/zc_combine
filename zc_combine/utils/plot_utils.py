...
def get_stats_ranks(dfs):
    ranks = {t: {k: v for k, v in d['all'].items() if 'ranking' in k} for t, d in dfs.items()}
    stats = {t: {k: v for k, v in d['all'].items() if k != 'index' and 'ranking' not in k} for t, d in dfs.items()}
